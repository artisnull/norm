...
export default function _normalizeSubNodes(dataSlice: object, node: Node) {
  const {
    subNodes,
    name,
    resolve,
    ...
    if (!subNode) {
      ...
      if (subSlice) {
        ...
        !subNode.omit &&
          this._addNormData({
            ...formattedData,
            nodeName: subNode.name || subNode.rename,
          })

        // normalize the subNode
        this.nodes.has(subNodeName) && this._normalizeNode(subSlice, subNode)
      } else {
        !this.silent &&
          console.warn(
            `Couldn't locate ${subNodeName} in ${rename ||
